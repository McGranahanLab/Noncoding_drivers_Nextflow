#!/usr/bin/env Rscript
# FILE: 2a_write_mutations_to_dndscv.R -------------------------------------
#
# DESCRIPTION: Formats MAF file containing mutations for one tumor subtype to
#              dNdScv input format.
#
# USAGE: Rscript --vanilla 2a_write_mutations_to_dndscv.R \
...
#
# OPTIONS: Run 
#          Rscript --vanilla  2a_write_mutations_to_dndscv.R -h
...
colsToGet <- c('participant_id', 'chr', 'start', 'ref', 'var')
...
message('[', Sys.time(), '] Formatting mutations for dNdScv, ', 
        args$cancer_subtype)
outfile <- paste0(args$output, '/dndscv-inputMutations-', args$cancer_subtype, 
