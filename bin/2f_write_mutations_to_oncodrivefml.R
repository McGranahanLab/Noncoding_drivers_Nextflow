...
#
# OPTIONS: Run 
#          Rscript --vanilla  2a_write_mutations_to_oncodrivefml.R -h
...
parser <- ArgumentParser(prog = 'write_mutations_to_oncodrivefml.R')
...
message('[', Sys.time(), '] Formatting mutations for OncodriveFML, ', 
        args$cancer_subtype)
outfile <- paste0(args$output, '/oncodrivefml-inputMutations-', args$cancer_subtype, 
...
write.table(maf, outfile, col.names = printColnames, row.names = F, sep = '\t',
            quote = F)
message('[', Sys.time(), '] Wrote mutations for OncodriveFML, ', 
        args$cancer_subtype)
