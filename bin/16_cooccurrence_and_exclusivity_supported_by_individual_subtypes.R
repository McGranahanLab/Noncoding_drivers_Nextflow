...
# Functions -------------------------------------------------------------------
#' check_significance_indiv_tumorType
...
check_significance_indiv_tumorType <- function(gene_pair,
  ...
  if (nrow(indivDiscover) != 0) {
    if (min(indivDiscover$discover.p.value_subtypeBiasRemoved, 
            na.rm = T) <= p_val_max) {
      result <- T
    }
  }
...
parser$add_argument("-d", "--discover", required = T, 
                    type = 'character', help = discoverHelp)

padjHelp <- paste('Cut off on adjusted p-value for significance of mutual ',
                  'incompatibility or co-occurence in histologically ',
                  'composite subtype')
parser$add_argument("-padj", "--p_adj_max_comp_subtype", required = T, 
                    type = 'character', help = padjHelp)
...
parser$add_argument("-du", "--discover_result_uniformal_subtypes", nargs = '+',
...
if (length(args$uniformal_tumor_subtypes) != 
    length(args$discover_result_uniformal_subtypes)) {
...
#              discover_result = '',  p_adj_max_comp_subtype = 0.1,
#              uniformal_tumor_subtypes = '',
#              discover_result_uniformal_subtypes = '',
#              p_max_unif_subtype = 0.1, output = '')

# Read DISCOVER results for composite subtype ---------------------------------
discover <- fread(args$discover_result, header = T, stringsAsFactors = F)
message('[', Sys.time(), '] Read DISCOVER results for composite subtype ',
        args$cancer_subtype, ": ", args$discover_result)

# Read DISCOVER results for histologically uniform subtypes -------------------
discoverUnifSubtypes <- lapply(args$discover_result_uniformal_subtypes, 
...
message('[', Sys.time(), '] Read DISCOVER results for uniformal subtypes ',
        ...
        ": ", paste0(args$discover_result_uniformal_subtypes, collapse = ', '))

# Check that linked genomic elements are supported by signal from histologically uniform subtypes ----
discover[, support_by_indivTT := F]
isSignificant <- discover$discover.p.adj_subtypeBiasRemoved <= args$p_adj_max_comp_subtype
if (any(isSignificant, na.rm = T)) {
  discover[isSignificant]$support_by_indivTT <- apply(discover[isSignificant], 1,
                                                      check_significance_indiv_tumorType,
                                                      discoverUnifSubtypes,
                                                      args$p_max_unif_subtype)
}
